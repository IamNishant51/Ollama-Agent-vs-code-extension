...
export function activate(context: vscode.ExtensionContext) {
  ...
  let latestThreadsState: { threads: any[]; currentId: string; historyOpen?: boolean; autoHideHistory?: boolean; compactTables?: boolean; stripedTables?: boolean } | undefined = context.globalState.get('ollamaAgent.threadsState');

  // Build initial project index in background
  const enableIndex = vscode.workspace.getConfiguration('ollamaAgent').get<boolean>('enableProjectIndex', true);
  if (enableIndex && vscode.workspace.workspaceFolders?.length) {
    ProjectIndexer.build().then(() => {
      console.log('[ollama-agent] ' + ProjectIndexer.getStats());
    });
  }
  ...
  const enableHover = vscode.workspace.getConfiguration('ollamaAgent').get<boolean>('enableExplainOnHover', false);
  if (enableHover) {
  ...
  context.subscriptions.push(vscode.languages.registerCodeActionsProvider({ scheme: 'file' }, new (class implements vscode.CodeActionProvider {
    provideCodeActions(document: vscode.TextDocument, range: vscode.Range, context: vscode.CodeActionContext): vscode.ProviderResult<(vscode.CodeAction | vscode.Command)[]> {
  ...
  context.subscriptions.push(vscode.commands.registerCommand('ollamaAgent.fixProblemHere', async (uriStr: string, range: vscode.Range, message: string) => {
  ...
  const enableDocStrings = vscode.workspace.getConfiguration('ollamaAgent').get<boolean>('enableDocstringCompletion', false);
  if (enableDocStrings) {
  ...
  context.subscriptions.push(vscode.commands.registerCommand('ollamaAgent.extractFunction', async () => {
    ...
    if (editor.selection.isEmpty) { vscode.window.showInformationMessage('Select code to extract.'); return; }
  ...
  context.subscriptions.push(vscode.commands.registerCommand('ollamaAgent.translateSelection', async () => {
    const editor = vscode.window.activeTextEditor; if (!editor) { return; }
    if (editor.selection.isEmpty) { vscode.window.showInformationMessage('Select text to translate.'); return; }
  ...
  context.subscriptions.push(vscode.commands.registerCommand('ollamaAgent.generateReadme', async () => {
    try {
      ...
      if (!ws) { vscode.window.showErrorMessage('Open a workspace to generate README.'); return; }
      // Build a deeper snapshot using chat panel helper-equivalent logic
      // Fallback to indexer summary for stats
      await ProjectIndexer.build(2000, 200);
      const stats = ProjectIndexer.getStats();
      const snapshot = ProjectIndexer.summary(true).slice(0, 70000);
      const models = await client.listModels();
      ...
      await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Generating README' }, async () => {
        await client.generate(model, prompt, true, (t: string) => { out += t; });
      });
      const readmeUri = vscode.Uri.joinPath(ws.uri, 'README.md');
      let original = '';
      try { original = Buffer.from(await vscode.workspace.fs.readFile(readmeUri)).toString('utf8'); } catch { original = ''; }
  ...
  context.subscriptions.push(
    vscode.languages.registerCodeLensProvider({ scheme: 'file' }, new (class implements vscode.CodeLensProvider {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ollamaAgent.addFileHere', async () => {
      const editor = vscode.window.activeTextEditor;
      const ws = vscode.workspace.workspaceFolders?.[0];
      if (!editor && !ws) { vscode.window.showErrorMessage('Open a file or a workspace first.'); return; }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ollamaAgent.addFileAI', async () => {
      const editor = vscode.window.activeTextEditor;
      const ws = vscode.workspace.workspaceFolders?.[0];
      if (!editor && !ws) { vscode.window.showErrorMessage('Open a file or a workspace first.'); return; }
      const baseDir = editor ? vscode.Uri.joinPath(editor.document.uri, '..') : ws!.uri;
      ...
      await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Generating file content' }, async () => {
        await client.generate(chosen, prompt, true, (t: string) => { out += t; });
      });
      const target = vscode.Uri.joinPath(ws?.uri ?? baseDir, relPath.replace(/^\/+/, ''));
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ollamaAgent.indexProject', async () => {
      await vscode.window.withProgress({ location: vscode.ProgressLocation.Window, title: 'Building project index…' }, async () => {
        await ProjectIndexer.build();
      });
      vscode.window.showInformationMessage(ProjectIndexer.getStats());
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('ollamaAgent.showIndexStats', async () => {
      vscode.window.showInformationMessage(ProjectIndexer.getStats());
    })
  );

  // Brushes
  const runBrush = async (instruction: string) => {
    await vscode.commands.executeCommand('ollamaAgent.inlineEdit', instruction);
  };
  context.subscriptions.push(
    vscode.commands.registerCommand('ollamaAgent.brush.simplify', () => runBrush('Simplify the selected code while preserving behavior.')),
    vscode.commands.registerCommand('ollamaAgent.brush.addTypes', () => runBrush('Add comprehensive static types and JSDoc/TS types to the selected code.')),
    vscode.commands.registerCommand('ollamaAgent.brush.optimize', () => runBrush('Optimize performance and memory usage of the selected code without changing external behavior.')),
  );

  // PR helpers
  context.subscriptions.push(
    vscode.commands.registerCommand('ollamaAgent.prSummary', async () => {
      try {
        const cwd = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
        if (!cwd) { vscode.window.showErrorMessage('Open a git workspace.'); return; }
        let diff = '';
        try { diff = (await exec('git --no-pager diff --cached', { cwd })).stdout; } catch {}
        if (!diff.trim()) { try { diff = (await exec('git --no-pager diff', { cwd })).stdout; } catch {} }
        if (!diff.trim()) { vscode.window.showInformationMessage('No git changes to summarize.'); return; }
        const models = await client.listModels();
        const chosen = await vscode.window.showQuickPick(models, { placeHolder: 'Select a model' });
        if (!chosen) { return; }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ollamaAgent.prReviewComments', async () => {
      try {
        const cwd = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
        if (!cwd) { vscode.window.showErrorMessage('Open a git workspace.'); return; }
        let diff = '';
        try { diff = (await exec('git --no-pager diff --cached', { cwd })).stdout; } catch {}
        if (!diff.trim()) { try { diff = (await exec('git --no-pager diff', { cwd })).stdout; } catch {} }
        if (!diff.trim()) { vscode.window.showInformationMessage('No git changes to review.'); return; }
        const models = await client.listModels();
        const chosen = await vscode.window.showQuickPick(models, { placeHolder: 'Select a model' });
        if (!chosen) { return; }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ollamaAgent.openReadmeMode', async () => {
      chatPanel.show(vscode.ViewColumn.Beside);
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ollamaAgent.threadsUpdate', async (payload: { threads: any[]; currentId: string; historyOpen?: boolean; autoHideHistory?: boolean; compactTables?: boolean; stripedTables?: boolean }) => {
      try {
        latestThreadsState = {
        ...
        try { (chatProvider as any)?.postMessage?.({ type: 'threadsState', ...latestThreadsState }); } catch {}
        // also push to chat panel webview if visible
        try { chatPanel.postMessage({ type: 'threadsState', ...(latestThreadsState || { threads: [], currentId: '' }) }); } catch {}
      } catch {}
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('ollamaAgent.requestThreads', async () => {
      if (!latestThreadsState) {
        latestThreadsState = context.globalState.get('ollamaAgent.threadsState');
      }
      const payload = latestThreadsState || { threads: [], currentId: '' } as any;
      try { (chatProvider as any)?.postMessage?.({ type: 'threadsState', ...payload }); } catch {}
      try { chatPanel.postMessage({ type: 'threadsState', ...payload }); } catch {}
    })
  );

  // Actions from left view to control chat webview threads
  context.subscriptions.push(
    vscode.commands.registerCommand('ollamaAgent.switchThread', async (id: string) => {
      chatPanel.show(vscode.ViewColumn.Beside);
      chatPanel.postMessage({ type: 'switchThread', id });
    }),
    vscode.commands.registerCommand('ollamaAgent.deleteThread', async (id: string) => {
      chatPanel.show(vscode.ViewColumn.Beside);
